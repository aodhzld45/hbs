...
const BoardManager: React.FC = () => {
  ...
  useEffect(() => {
    const loadBoardMeta = async () => {
      try {
        ...
        const config = await fetchBoardConfigByCode(normalizedBoardCode);
  ...
  const isPageLoading = isBoardMetaLoading || isLoading;
  ...
  const handleExcelDownload = async () => {
    try {
      ...
      const filename = (() => {
        if (!rawHeader) return `${normalizedBoardCode}.xlsx`;
