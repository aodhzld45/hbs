...
- local/prod 테스트 페이지를 `/sdk/v1` 배포 단위 안으로 분리했다.
  - local: `hsb-fe/public/sdk/v1/sdk-test-local.html`
  - prod: `hsb-fe/public/sdk/v1/sdk-test-prod.html`
...
- complete4 자동 재시도 정책을 지수 backoff와 최대 시도 횟수 기준으로 표준화했다.
- `retryMaxAttempts`, `retryBaseDelayMs`, `retryMaxDelayMs`, `retryOnStatusCodes` 옵션을 추가했다.
- 운영 장애별 error code를 `CHAT_UNAUTHORIZED`, `CHAT_FORBIDDEN`, `CHAT_SERVER_ERROR`, `CHAT_TIMEOUT`, `CHAT_NETWORK_ERROR`, `QUOTA_EXCEEDED` 등으로 세분화했다.
- `/sdk/v1/hsbs-manifest.json`으로 SHA-384/SHA-256 hash manifest를 제공한다.
- `/sdk/v1/README.md`에 CSP 가이드, SRI 설치 예시, retry 운영 옵션, 장애 코드 표준을 추가했다.

### 작업

- [완료] 네트워크 timeout과 retry 정책을 분리한다.
  - [완료] ping/widget-config: 짧은 timeout, 실패 시 위젯 미노출
  - [완료] complete4: 사용자 안내 메시지 + 재시도 버튼
  - [완료] complete4: 지수 backoff 기반 자동 retry와 최대 시도 횟수 옵션
...
- [완료] CSP 가이드를 문서화한다.
  ...
  - `connect-src`
  - `img-src`
- [완료] SRI 또는 SDK hash manifest를 제공한다.
...
- 상태: 시작
- 반영일: 2026-05-27
- 메인 포트폴리오 페이지에서 기존 React Assistant 대신 `/sdk/v1/hsbs-chat.js` 공식 SDK 위젯을 마운트하도록 전환했다.
- `options.sizePreset` 기반 크기 프리셋을 추가했다.
  - `compact`
  - `standard`
  - `large-portfolio`
- `large-portfolio`는 기존 포트폴리오 AssistantWidget과 유사하게 데스크톱 큰 버블, 데스크톱 고정 패널, 모바일 풀스크린을 지원한다.
- WidgetConfig 관리자 화면에서 DB 테이블 변경 없이 `options_json` 기반으로 데스크톱/모바일 버블 크기, 데스크톱 패널 너비/높이, 모바일 풀스크린을 설정할 수 있게 했다.
...
- [진행 중] WidgetConfig를 디자인 토큰 중심으로 재정리한다.
