...
현재 `hsb-fe/public/sdk/prod/hsbs-chat_prod.js` 기준으로 다음 흐름이 구현되어 있습니다.
...
- public SDK 기준 파일을 하나로 통일한다.
- 현재 `hsb-fe/public/hsbs-chat_prod.js`와 `hsb-fe/public/sdk/prod/hsbs-chat_prod.js`가 별도로 존재하므로 배포 기준을 `/sdk/prod/hsbs-chat_prod.js` 또는 `/sdk/v1/hsbs-chat.js`로 단일화한다.
- 루트의 중복 파일은 legacy redirect 또는 build 산출물로만 유지한다.
- SDK 엔드포인트를 문서화한다.
  - `GET /api/ai/ping`
  - `GET /api/ai/public/widget-config`
  - `GET /api/ai/public/prompt-profile`
  - `POST /api/ai/complete4`
- BO에서 SiteKey, defaultWidgetConfig, defaultPromptProfile 연결이 비어 있으면 위젯이 뜨지 않는다는 운영 체크리스트를 추가한다.
- SDK 테스트 페이지를 prod/local로 분리하고, 테스트 siteKey와 운영 siteKey를 명확히 구분한다.
...
- CDN/정적 경로를 버전 기반으로 고정한다.
  - `/sdk/v1/hsbs-chat.js`
  - `/sdk/v1/hsbs-chat.css`
  - `/sdk/v1/latest.js`
  - `/sdk/v1.2.0/hsbs-chat.js`
- loader와 core를 분리한다.
  - `hsbs-loader.js`: siteKey, apiBase, version, theme를 읽고 실제 SDK/CSS 로드
  - `hsbs-chat.iife.js`: 브라우저 전역 `window.HSBS` 제공
...
- 네트워크 timeout과 retry 정책을 분리한다.
  - ping/widget-config: 짧은 timeout, 실패 시 위젯 미노출
  - complete4: 사용자 안내 메시지 + 재시도 버튼
- 장애 fallback 메시지를 WidgetConfig에서 설정 가능하게 한다.
- offline 감지와 `navigator.onLine` 기반 안내를 추가한다.
- 중복 init, destroy, re-init을 안정화한다.
- CSP 가이드를 문서화한다.
  - `script-src`
  - `style-src`
  - `connect-src`
  - `img-src`
- SRI 또는 SDK hash manifest를 제공한다.
- debug 모드는 고객 콘솔을 오염시키지 않도록 명시 옵션일 때만 상세 로그를 남긴다.
